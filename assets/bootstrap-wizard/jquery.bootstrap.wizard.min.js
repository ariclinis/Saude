...
var f=null;var h=null;
this.fixNavigationButtons=function(s){

	if(!f.length){h.find("a:first").tab("show");
f=h.find("li:first");}
if(g.firstIndex()>=g.currentIndex()){b("li.previous",e).addClass("disabled");}else{b("li.previous",e).removeClass("disabled");}
if(g.currentIndex()>=g.navigationLength()){b("li.next",e).addClass("disabled");
}else{
...
if(c.onShow&&typeof c.onShow==="function"){c.onShow(f,h,g.nextIndex());}g.fixNavigationButtons();b('a[data-toggle="tab"]',e).on("click",function(i){if(c.onTabClick&&typeof c.onTabClick==="function"&&c.onTabClick(f,h,g.currentIndex())===false){return false;
	...
	if($element.hasClass("disabled")){
		return false;
	}f=$element;g.fixNavigationButtons();
...
}return this.each(function(e){var f=b(this);if(f.data("bootstrapWizard")){return;}var g=new a(f,d);f.data("bootstrapWizard",g);});};
b.fn.bootstrapWizard.defaults={tabClass:"nav nav-pills",nextSelector:".wizard li.next",previousSelector:".wizard li.previous",firstSelector:".wizard li.first",lastSelector:".wizard li.last",onInit:null,onNext:null,onPrevious:null,onLast:null,onFirst:null};
})(jQuery);
